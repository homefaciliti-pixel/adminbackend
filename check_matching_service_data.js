...
async function run() {
  ...
  try {
    const [nodeRows] = await connection.query("SELECT * FROM node_services WHERE title LIKE '%Professional Compounders%'");
    console.log('--- in node_services ---');
    console.log(JSON.stringify(nodeRows, null, 2));

    const [normalRows] = await connection.query("SELECT * FROM services WHERE title LIKE '%Professional Compounders%'");
    console.log('\n--- in services (non-prefixed) ---');
    console.log(JSON.stringify(normalRows, null, 2));
